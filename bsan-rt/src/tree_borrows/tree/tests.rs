...
mod spurious_read {
    ...
    impl LocStateProtPair {
            ...
            RetY: Exhaustive + 'static,
        {
            ...
            let mut seen = FxHashSet::<(LocStateProtPair, LocStateProtPair)>::default();
        ...
        for pat in Pattern::exhaustive() {
                ...
                initial_source.all_states_reachable_via_opaque_code::</*X*/ NoRet, /*Y*/ AllowRet>()
            {
                ...
                if let Some(final_target) = distinguishable {
                    ...
                    crate::eprintln!("  (arbitrary code instanciated with '{opaque}')");
                    err += 1;
...
fn dead_leaf_is_removed_and_zeroed() {
    ...
    let mut dead = [t(11)];
    let empty = tree.remove_dead_tags(&mut dead, AllocId::default());
...
fn retained_tag_is_pruned_once_children_die() {
    ...
    let mut dead = [t(11)];
    assert!(!tree.remove_dead_tags(&mut dead, AllocId::default()));
    assert_eq!(dead, [t(11)]);
