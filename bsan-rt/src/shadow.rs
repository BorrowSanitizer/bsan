    ...
    pub fn memcpy(&self, hooks: &BsanHooks, src: usize, dst: usize, num_bytes: usize) {
        ...
        while words_remaining > 0 {
            unsafe {
                ...
                let dst: *mut T = unsafe { &raw mut (*l2_table_dst.as_ptr())[dst_index.l2_index] };

                if let Some(l2_table_src) = l2_table_src {
                    let src: *mut T =
                        unsafe { &raw mut (*l2_table_src.as_ptr())[src_index.l2_index] };
...
mod tests {
    ...
    fn memcpy() {
        ...
        let dst_address = unsafe { (L2_LEN - (halfmax / 2)) * PTR_BYTES };
