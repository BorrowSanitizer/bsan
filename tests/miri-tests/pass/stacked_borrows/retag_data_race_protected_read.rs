//@run:0
//miri: @compile-flags:-Zmiri-deterministic-concurrency
use std::thread;
...
fn main() {
    ...
    let t = thread::spawn(move || {
        ...
        fn retag(_x: &mut i32) {}
        retag(unsafe { &mut *ptr.0 }); //miri: ~ERROR: Data race detected between (1) non-atomic read on thread `main` and (2) retag write of type `i32` on thread `unnamed-1`    
    });
