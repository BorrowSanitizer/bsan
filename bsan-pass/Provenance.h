...
// Each component of provenance needs to be representable as either a scalar or
// a vector of these components.
template <typename ScalarType, typename VectorType> class ScalarOrVector {
public:
  ...
  ProvenanceKind getKind() const {
    return std::holds_alternative<ScalarType>(data_) ? ProvenanceKind::Scalar
  ...
  ScalarType assertScalar() const { return this->getScalar().value(); }
  ...
  VectorType assertVector() const { return this->getVector().value(); }
...
struct ProvenanceVector {
  ...
  ProvenanceVector() {}
  ProvenanceVector(Value *I, Value *T, Value *F, Value *L, ElementCount E)
      : IdVector(I), TagVector(T), InfoVector(F), Length(L), Elems(E) {}
  bool operator==(const ProvenanceVector &other) const {
    return this->IdVector == other.IdVector &&
           ...
           this->Length == other.Length && this->Elems == other.Elems;
  }
  bool operator!=(const ProvenanceVector &other) const {
    return !(*this == other);
  }
...
struct ProvenancePointer {
  ...
  Value *Length;
  ElementCount Elems;
  ProvenancePointer(Value *B, Value *L, ElementCount E)
      : Base(B), Length(L), Elems(E) {}

  bool isVector() { return Elems.isScalable(); }
};
