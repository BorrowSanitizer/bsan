...
namespace
{
    ...
    // otherwise we would risk invalidating the lifetimes of existing instructions during
    // the pass. This is equivalent to LLVM's `EscapeEnumerator`, but we cannot reuse that,
    // since our retag intrinsics are currently set to unwind, so they're accidentally caught
    // in the transformation. In the future, when we adjust that behavior, we might consider
    // replacing this with `EscapeEnumerator`.
    struct ThrowingCallTransformer : public InstVisitor<ThrowingCallTransformer> {
        ...
        ThrowingCallTransformer(Function &F, BorrowSanitizer &BS, const TargetLibraryInfo *TLI, DominatorTree &DT)
    ...
    struct BorrowSanitizerVisitor : public InstVisitor<BorrowSanitizerVisitor> {
        ...
        // We iterate over instructions in chunks for each block corresponding to a depth-first
        // traversal of the CFG. This is necessary to maintain per-block metadata for the provenance
        // of stack allocations.
        SmallVector<std::tuple<BasicBlock *, SmallVector<Instruction *>>> Instructions;
        // If a stack allocation does not have a dedicated `lifetime.start`, then we allocate metadata for it
        // within the entry block. We use a liveness pass to determine which allocations need to be freed,
        // so no additional handling is necessary to determine where to free these allocations, even if they
        // do not have a `lifetime.end`, either.
        ...
        // The number of function-entry retags. If none occur, then we can skip creating and popping
        // a frame to contain protected tags.
        ...
        // With the exception of `allocas`, each value is associated with a unique provenance value.
        // Provenanced values are indexed by each provenance carrying component. For example,
        // if `ProvenanceComponents[V]` has length 3, then `ProvenanceMap[std::make_pair(V, 2)]` 
        // would return the third provenance value within `V`.
        ProvenanceMap BaseProvMap;
    
        // Most allocations have a single `lifetime.start`. We assign a single provenance value to these
        // allocations starting from the entry block. It is left uninitialized until the `lifetime.start`.
        // Uninitialized provenance values have the same semantics as invalid ones, so we can still detect
        // UB for accesses outside of the lifetime. This is necessary; otherwise, ~thousands~ of PHI nodes
        // can be emitted for certain edge-case functions. 
        DenseMap<AllocaInst *, std::pair<Value *, ProvenanceScalar>> SingletonAllocaMap;


        // If an `alloca` has multiple `lifetime.start` instructions, then we need to track each one
        // separately, because any access might be mutually dominated by more than one `lifetime.start`. 
        DenseMap<BasicBlock *, DenseMap<AllocaInst *, ProvenanceScalar>> AllocaProvMap;

        // Sometimes, a GEP is issued for an alloca before its `lifetime.start`. The Rust-view of `lifetime.start`
        // indicates that the result of this GEP should be invalid, but the LLVM view seems to permit this. For now,
        // we defer initializing the provenance of a GEP for an `alloca` until we need to use it to validate an operation.
        // Instead of setting the provenance for these GEPs, we indicate in this map that they alias an `alloca`.
        // Then, when we need to get the provenance for the GEP, we look to see if it's an alias for an `alloca`. 
        // If so, we return the provenance for the `alloca` based on whichever block that we're instrumenting. This interaction
        // is only necessary for the edge cases where the `alloca` has multiple `lifetime.start`. 
        DenseMap<Value *, AllocaInst *> AllocaAliases;
        
        // If a PHI node is a pointer or a vector of pointers, then we need to emit corresponding "shadow" PHI nodes for
        // its provenance. To emit these PHI nodes, we need to know the provenance for each argument at each incoming block. 
        // We only have this information once we have finished instrumenting each block. So, we temporarily store our shadow PHI 
        // nodes and wait until the end of the pass to "patch in" the missing provenance values. While these values are pending, 
        // the PHI node will contain "wildcard" provenance values. This is necessary since multiple LLVM APIs are implemented under the
        // assumption that any arbitrary PHI node will have at least one incoming block, and that all incoming values will be initialized.
        // Leaving a PHI node in an  invalid state in the middle of the instrumentation pass inevitably leads to memory corruption.
        SmallVector<std::tuple<PHINode *, Provenance, unsigned int>> ProvPHINodes;

        // Since `allocas` have multiple provenance values, the provenance for any given `alloca` in a particular block will depend on 
        // all of its incoming blocks.
        ...
        // We use LLVM's lifetime analysis to determine which `allocas` are alive at every exit point.
        std::unique_ptr<StackLifetime> LifetimeInfo;
        
        BorrowSanitizerVisitor(Function &F, BorrowSanitizer &BS, const StackSafetyGlobalInfo *const SSGI, const TargetLibraryInfo &TLI, DominatorTree &DT)
            : F(F), BS(BS), DIB(*F.getParent(), /*AllowUnresolved*/ false), C(BS.C), TLI(&TLI), SSGI(SSGI), CurrentBlock(&F.getEntryBlock()), DT(DT)
        {}
        ...
        ProvenanceVector assertProvenanceVector(IRBuilder<> &IRB, Value *V, ElementCount E) {
            return assertProvenanceVector(IRB, {V, 0}, E);
        }

        ProvenanceVector assertProvenanceVector(IRBuilder<> &IRB, ProvenanceKey Key, ElementCount E) {
        ...
        // If no provenance has been assigned yet, then the null provenance value is returned.
        ProvenanceVector assertProvenanceVector(IRBuilder<> &IRB, BasicBlock *BB, ProvenanceKey Key, ElementCount E) {
            std::optional<Provenance> OptProv = getProvenance(BB, Key);
            if (OptProv.has_value()) {
                Provenance Prov = OptProv.value();
                if (Prov.isVector()) {
                    ...
                    report_fatal_error("Expected vector provenance, but found scalar provenance!");
                }
            } else {
        ...
        Provenance assertProvenance(IRBuilder<> &IRB, ProvenanceComponent &Comp, Value *V) {
            return assertProvenance(IRB, Comp, {V, 0});
        }

        Provenance assertProvenance(IRBuilder<> &IRB, ProvenanceComponent &Comp, ProvenanceKey Key) {
        ...
        Provenance assertProvenance(IRBuilder<> &IRB, BasicBlock *BB, ProvenanceComponent &Comp, ProvenanceKey Key) {
            std::optional<Provenance> OptProv = getProvenance(BB, Key);
            if (OptProv.has_value()) {
                Provenance Prov = OptProv.value();
        ...
        // Asserts that there is either a provenance value at the given index, or that no provenance
        // values have been loaded for the given value. Does not return the null provenance value.
        ...
        std::optional<Provenance> getProvenance(BasicBlock *BB, ProvenanceKey Key) {
            if(BaseProvMap.contains(Key)) {
            ...
            }else if(AllocaAliases.contains(Key.first)){
                AllocaInst *AI = AllocaAliases[Key.first];
                return getProvenance(BB, {AI, 0});
            }else if(AllocaInst *AI = dyn_cast<AllocaInst>(Key.first)){
                return getAllocaProvenance(BB, AI);
            } else if(Argument *Arg = dyn_cast<Argument>(Key.first)) {
                ...
                if (ArgumentProvenance.count(Arg)) {
                    ...
                    ProvenancePointer ArgProvenancePtr = ArgumentProvenance[Arg][Key.second];
        ...
        ProvenanceScalar getAllocaProvenanceRecurse(BasicBlock *BB, AllocaInst *AI, DenseSet<BasicBlock *> &Visited) {
        ...
        void setProvenance(Value *V, Provenance Prov) {
            setProvenance({V, 0}, Prov);
        }
        ...
        SmallVector<ProvenanceComponent> *getProvenanceComponents(IRBuilder<> &IRB, Type *Ty) {
            if (!ProvenanceComponents.contains(Ty))
                populateProvenanceComponents(IRB, ProvenanceComponents[Ty], Ty, Ty, BS.Zero, BS.Zero);
        ...
        std::tuple<Type *, uint64_t> offsetIntoProvenanceIndex(
            IRBuilder<> &IRB, 
            Type *CurrentTy, 
            uint64_t Idx, 
        ...
        void storeProvenanceToShadow(IRBuilder<> &IRB, Value *ObjAddr, Provenance Prov) {
            if (Prov.isVector()) {
        ...
        Provenance loadProvenanceFromShadow(IRBuilder<> &IRB, ProvenanceComponent &Comp, Value *ObjAddr) {
            if (Comp.isVector()) {
                ...
                std::tie(IdVector, TagVector, InfoVector) = allocateProvenanceVectors(IRB, Comp.Elems);
                ...
                return loadProvenanceVector(IRB, IdVector, TagVector, InfoVector, Comp.NumProvenanceValues, Comp.Elems);
            } else {
        ...
        Provenance loadProvenance(IRBuilder<> &IRB, ProvenancePointer Prov) {
            if (Prov.isVector()) {
                ...
                std::tie(Id, Tag, Info, Next) = getProvenanceVectorElements(IRB, Prov.Base, Prov.Elems);
        ...
        ProvenanceVector loadProvenanceVector(
            IRBuilder<> &IRB, 
            Value *IdPtr, 
            Value *TagPtr, 
            Value *InfoPtr, 
        ...
        std::tuple<Value *, Value *, Value *> allocateProvenanceVectors(IRBuilder<> &IRB, ElementCount Elems) {
        ...
        ProvenanceVector wildcardProvenanceVector(IRBuilder<> &IRB, ElementCount Elems) {
        ...
        void initStack() {
            ...
            for (auto &Arg : F.args()) {
                SmallVector<ProvenanceComponent> *Components = getProvenanceComponents(EntryIRB, Arg.getType());
                for (auto &C : *Components) {
                    Value *CurrentArrayByteOffset = EntryIRB.CreateMul(TotalNumProvenanceValues, BS.ProvenanceSize);
                    Value *CurrentArraySlot = offsetPointer(EntryIRB, BS.ParamTLS, CurrentArrayByteOffset);
                    ProvenancePointer Ptr = C.getPointerToProvenance(EntryIRB, CurrentArraySlot);
            ...
            LifetimeInfo = std::make_unique<StackLifetime>(F, StaticAllocaVec, StackLifetime::LivenessType::May);
            LifetimeInfo->run();
        }
            ...
            for (const auto &[PN, Prov, Idx] : ProvPHINodes) { 
                if(Prov.isVector()) {
                    ...
                    for (auto [V, IncomingBlock] : llvm::zip(PN->incoming_values(), PN->blocks())) {
                        ...
                        InfoNode->setIncomingValueForBlock(IncomingBlock, IncomingProv.InfoVector);
                        LengthNode->setIncomingValueForBlock(IncomingBlock, IncomingProv.Length);
                    }
                }else{
        ...
        ProvenanceScalar instrumentHeapAllocation(IRBuilder<> &IRB, Value *Address, Value *Size) {
        ...
        void instrumentDeallocation(IRBuilder<> &IRB, Value *Ptr) {
            ...
            IRB.CreateCall(BS.BsanFuncDealloc, {Ptr, Prov.Id, Prov.Tag, Prov.Info, BS.True});
        }
        ...
        void visitCallBase(CallBase &CB) {
            ...
            } else if (Callee && Callee->getName().starts_with(kBsanPrefix)) {
            ...
            // The process for computing provenance components is deterministic, so we can guarantee
            ...
            for (const auto &[i, Arg] : llvm::enumerate(CB.args())) {
                SmallVector<ProvenanceComponent> *Components = getProvenanceComponents(Before, Arg->getType());
                for (const auto &[Idx, Comp] : llvm::enumerate(*Components)) {
                    TotalNumProvenanceValues = Before.CreateAdd(TotalNumProvenanceValues, Comp.NumProvenanceValues);
            ...
            if(!isRustShim(CB)){
                ...
                if (isAllocLikeFn(&CB, TLI)) {
                    ...
                    return;
                } else if (Value *Operand = getReallocatedOperand(&CB)) {
                    Value *Size = resolveAllocSize(After, CB);
                    instrumentHeapAllocation(After, &CB, Size);
                    instrumentDeallocation(After, Operand);
                    return;
                } else if (Value *Operand = getFreedOperand(&CB, TLI)) {
                    instrumentDeallocation(After, Operand);
                    return;
                }
            }
            ...
            if (CB.getType()->isSized()) {
                ...
                for (const auto &[Idx, Comp] : llvm::enumerate(*ReturnComponents)) {
                    ...
                    TotalNumReturnProvenanceValues = After.CreateAdd(Comp.NumProvenanceValues, TotalNumReturnProvenanceValues);
                    Value *ByteWidth = Before.CreateMul(Comp.NumProvenanceValues, BS.ProvenanceSize);
        ...
        ProvenanceVector createVectorProvenancePHI(IRBuilder<> &IRB, ElementCount Elems, iterator_range<pred_iterator> Blocks) {
            ...
            PHINode *TagNode = IRB.CreatePHI(IntptrVector, NumIncoming, "_bsphi_vec_tag");
            TagNode->dropDbgRecords();
            PHINode *InfoNode = IRB.CreatePHI(PtrVector, NumIncoming, "_bsphi_vec_info");
            InfoNode->dropDbgRecords();
            PHINode *LengthNode = IRB.CreatePHI(BS.IntptrTy, NumIncoming, "_bsphi_vec_len");
        ...
        ProvenanceScalar createScalarProvenancePHI(IRBuilder<> &IRB, iterator_range<pred_iterator> Blocks) {
            unsigned NumIncoming = std::distance(Blocks.begin(), Blocks.end());
        ...
        void visitPHINode(PHINode &PN) {
            ...
            for (auto [Idx, Comp] : llvm::enumerate(*Components)) {
                Provenance Prov = createProvenancePHI(IRB, Comp, predecessors(PN.getParent()));
        ...
        std::pair<Value *, ProvenanceScalar> createAllocaMetadata(IRBuilder<> &IRB, AllocaInst *AI){
        ...
        void initAllocaMetadata(IRBuilder<> &IRB, AllocaInst *AI, Value *Size, ProvenanceScalar Prov) {
            IRB.CreateCall(BS.BsanFuncAllocStack, {AI, Size, Prov.Id, Prov.Tag, Prov.Info});
        }

        ProvenanceScalar createAndInitAllocaMetadata(IRBuilder<> &IRB, AllocaInst *AI){
        ...
        void visitExtractValueInst(ExtractValueInst &EI) {
            ...
            for (auto &Idx : EI.indices()) {
                std::tie(CurrType, StartingIdx) = offsetIntoProvenanceIndex(IRB, CurrType, Idx, StartingIdx);
            }

            for (auto [Offset, Comp] : llvm::enumerate(*DestComponents)) {
                Provenance Prov = assertProvenance(IRB, Comp, {AggregateSrc, StartingIdx + Offset});
        ...
        void visitExtractElementInst(ExtractElementInst &EE) {
            ...
            if (SrcType->getElementType()->isPointerTy()) {
                ...
                ProvenanceVector VP = assertProvenanceVector(IRB, V, VT->getElementCount());
        ...
        void visitShuffleVectorInst(ShuffleVectorInst &SI) {
            ...
            if (SpecificTy->getElementType()->isPointerTy()) {
                ...
                ProvenanceVector VPL = assertProvenanceVector(IRB, LHS, LHT->getElementCount());

                Value *RHS = SI.getOperand(1);
                VectorType *RHT = dyn_cast<VectorType>(RHS->getType());
                ProvenanceVector VPR = assertProvenanceVector(IRB, RHS, RHT->getElementCount());
                ...
                setProvenance(&SI, ProvenanceVector(ShuffledIds, ShuffledTags, ShuffledInfo, Length, DestElems));
            }
        }
        ...
        void visitSelectInst(SelectInst &SI) {
            ...
            for (auto [Idx, Comp] : llvm::enumerate(*Components)) {
                if (Comp.isVector()) {
                    ...
                    ProvenanceVector ProvL = assertProvenanceVector(IRB, {SI.getTrueValue(), Idx},  Comp.Elems);
                    ProvenanceVector ProvR = assertProvenanceVector(IRB, {SI.getFalseValue(), Idx}, Comp.Elems);
                    ...
                    setProvenance({&SI, Idx}, ProvenanceVector(Id, Tag, Info, Len, Comp.Elems));
                } else {
                    // For scalable provenance, we just select on each of the three components.
                    ProvenanceScalar ProvL = assertProvenanceScalar({SI.getTrueValue(), Idx});
                    ProvenanceScalar ProvR = assertProvenanceScalar({SI.getFalseValue(), Idx});
...
static Constant *getOrInsertTLSGlobal(Module &M, StringRef Name, Type *Ty) {
    return M.getOrInsertGlobal(Name, Ty, [&] { 
        return new GlobalVariable(
            M, Ty, false, GlobalVariable::ExternalLinkage,
...
void BorrowSanitizer::initializeCallbacks(Module &M, const TargetLibraryInfo &TLI) {
    ...
    BsanFuncDealloc = M.getOrInsertFunction(
        ...
        IRB.getVoidTy(),
        PtrTy, IntptrTy, IntptrTy, PtrTy, Int8Ty);

    BsanFuncDeallocWeak = M.getOrInsertFunction(
        kBsanFuncDeallocWeakName, AL,
        IRB.getVoidTy(),
    ...
    BsanFuncDebugParamTLS = M.getOrInsertFunction(
        kBsanFuncDebugParamTLS, AL,
        IRB.getVoidTy(),
        IntptrTy);
    
    BsanFuncDebugRetvalTLS = M.getOrInsertFunction(
        kBsanFuncDebugRetvalTLS, AL,
        IRB.getVoidTy(),
        IntptrTy);

    EHPersonality Pers = getDefaultEHPersonality(TargetTriple);
    DefaultPersonalityFn = M.getOrInsertFunction(
        getEHPersonalityName(Pers),
...
void BorrowSanitizer::createUserspaceApi(Module &M, const TargetLibraryInfo &TLI) {
    IRBuilder<> IRB(*C);
    RetvalTLS = getOrInsertTLSGlobal(M, kBsanRetvalTLSName, ArrayType::get(ProvenanceTy, kTLSSize));
    ParamTLS = getOrInsertTLSGlobal(M, kBsanParamTLSName, ArrayType::get(ProvenanceTy, kTLSSize));
