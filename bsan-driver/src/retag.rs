...
use rustc_middle::mir::visit::MutVisitor as MirVisitor;
use rustc_middle::mir::{
    Body, Place, RetagKind, Rvalue, Statement, StatementKind, TerminatorKind, START_BLOCK,
};
use rustc_middle::ty::{self, Ty, TyCtxt};

use crate::*;

pub struct AddRetagPass;
impl<'tcx> AddRetagPass {
    pub fn run(ctx: &mut BSanCtx, compiler: &Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        // insert retag function into global context.
...
pub struct AddRetagVisitor<'a, 'tcx> {
    tcx: &'a TyCtxt<'tcx>,
}

impl<'tcx> MirVisitor<'tcx> for AddRetagVisitor<'_, 'tcx> {
    ...
    fn visit_body(&mut self, body: &mut Body<'tcx>) {
        ...
        for block_data in basic_blocks {
            ...
            for i in (0..block_data.statements.len()).rev() {
                        ...
                        let add_retag = match rvalue {
                            ...
                            Rvalue::RawPtr(_mutbl, place) => {
                                ...
                                if place.is_indirect_first_projection()
                                    && body.local_decls[place.local].ty.is_box_global(self.tcx())
                                {
                                    Some(RetagKind::Raw)
                                } else {
                                    None
                ...
                let _source_info = block_data.statements[i].source_info;
                // TODO: insert retag
            }
        }
    }
